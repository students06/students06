...
export const ClassesManagement: React.FC = () => {
  ...
  const getTeacherDisplayName = (teacherId: string) => {
    ...
    const subjectName = subject?.name || 'غير محدد';
    
    return `${teacher.name} - ${subjectName}`;
  };
          ...
          return (
            <div key={cls.id} className="bg-white rounded-lg shadow-sm border p-6 hover:shadow-md transition-shadow duration-200">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                ...
                <div>
                  <span>المعلم: {teacherDisplayName}</span>
                </div>
                <div>
                  <span>تاريخ الإنشاء: {new Date(cls.createdAt).toLocaleDateString('en-GB')}</span>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${(classStudents.length / cls.maxCapacity) * 100}%` }}
                  />
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  {((classStudents.length / cls.maxCapacity) * 100).toFixed(1)}% ممتلئ
                </p>
              </div>
