-- قاعدة بيانات نظام إدارة الحضور الشامل - النسخة المصححة
-- تاريخ التحديث: 2024-01-19
-- متوافقة مع MariaDB/MySQL
...
CREATE TABLE attendance (
    ...
    INDEX idx_date_timestamp (DATE(timestamp))
);
...
-- إنشاء Views مفيدة (بدون استخدام كلمات محجوزة)
...
-- إنشاء فهارس إضافية لتحسين الأداء (بدون استخدام DATE() في INDEX)
CREATE INDEX idx_attendance_date_status ON attendance (timestamp, status);
CREATE INDEX idx_sessions_date_status ON sessions (start_time, status);
CREATE INDEX idx_reports_date ON reports (created_at);
CREATE INDEX idx_whatsapp_logs_date_status ON whatsapp_logs (sent_at, status);

-- رسالة النجاح
SELECT 'تم إنشاء قاعدة البيانات المصححة بنجاح!' as message,
       'يمكنك الآن استخدام النظام مع قاعدة البيانات MySQL/MariaDB' as note;